...
class OrientedRCNN(RotatedTwoStageDetector):
    ...
    def forward_dummy(self, img):
        ...
        centers = torch.rand(num_proposals, 2, device=img.device)
        centers[:, 0] *= img_w
        centers[:, 1] *= img_h
        sizes = torch.rand(num_proposals, 2, device=img.device) * 120 + 8
        angles = torch.zeros(num_proposals, 1, device=img.device)
        proposals = torch.cat([centers, sizes, angles], dim=1)
        # roi_head
